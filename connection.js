...
// Button State Handler: Icon, Color aur Text update karta hai
function updateStartButton(state) {
    const btnSpan = startBtn.parentElement.querySelector('span') || document.getElementById('start-label');

    if (state === 'searching' || state === 'connected') {
        // Red State: Jab search ho raha ho ya call chal rahi ho
        ...
        if (btnSpan) btnSpan.innerText = "End Call";
    } else {
        // Green State: Jab idle ho
        ...
        if (btnSpan) btnSpan.innerText = "Start Call";
    }
}
...
socket.on('matched', async (data) => {
    ...
    // UI Update: Button ko "End Call" banayein
    updateStartButton('connected');
...
muteBtn.addEventListener('click', () => {
    ...
    if (muteLabel) muteLabel.innerText = isMuted ? "Unmute" : "Mute";
});